...
namespace WearMEBraceControlSystem
{
	...
	EposController::EposController(char * pCommPort, int n_id) : _COM_PORT(pCommPort)
	{
		...
		if (handle)
		{
			...
			VCS_ActivateProfilePositionMode(handle, node_id, &errorCode);
	...
	bool EposController::MoveTo(long position, long upperlimit, long lowerlimit)
	{
		bool complete = false;
		long pos = 0;
		if (handle != NULL)
		{
			if (position <= upperlimit && position >= lowerlimit ) //do not command an absolute position that is larger than the maximum absolute position, **TODO** determine the maximum position
			{
				ClearErrors(&handle, &errorCode);
				complete = VCS_MoveToPosition(handle, node_id, position, true, true, &errorCode);
				VCS_GetPositionIs(handle, node_id, &pos, &errorCode);
